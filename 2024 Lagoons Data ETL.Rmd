...
LagoonExists = function(DatabaseConnection, Lagoon) {
  tryCatch({
    ...
    if (nrow(result) == 0) {
      ...
      return(paste("SELECT * FROM Lagoons WHERE Lagoon = '",Lagoon,"') -- ",Lagoon," Exists\n",sep=""))
    }
  }, error = function(e) {
    # Bummer
    cat("Error: ", e$message,"\n",Sql,"\n")
...
SiteExists = function(DatabaseConnection, Lagoon,Site,Lat='NULL',Lon='NULL') {
  tryCatch({
    ...
    if (nrow(result) == 0) {
      ...
      return(paste("SELECT * FROM Sites WHERE Site = '",Site,"' -- ",Site," Exists\n",sep=""))
    }
...
SamplingEventExists = function(DatabaseConnection, Lagoon,Date,Site) {
  tryCatch({
    ...
    if (nrow(result) == 0) {
      ...
      return(paste("SELECT * FROM SamplingEvents WHERE Lagoon='",Lagoon,"' And Site = '",Site,"' And [Date] = '",Date,"' -- ",Site," Exists\n",sep=""))
    }
